...
function renderDashboard() {
  ...
  root.innerHTML = `<div class="case-info-header"><span class="info-tag"><i class="fa-solid fa-landmark"></i> Live Indian Kanoon Source</span><h2>${escapeHtml(selectedCase.name)}</h2><div class="metadata-grid"><div><strong>Court:</strong> ${escapeHtml(selectedCase.court)}</div><div><strong>Bench:</strong> ${escapeHtml(selectedCase.bench)}</div><div><strong>Year:</strong> ${escapeHtml(selectedCase.year)}</div><div><strong>Citation:</strong> ${escapeHtml(selectedCase.citation)}</div><div class="meta-wide"><strong>Judges:</strong> ${escapeHtml(selectedCase.judges)}</div></div></div><article class="case-overview"><h3>Case Overview</h3><p>${escapeHtml(selectedCase.overview)}</p>${citationLink(selectedCase.source_url, 'Open original judgment on Indian Kanoon')}</article><div class="case-sections">${sections.map(([title, text]) => `<details><summary>${title}</summary><div class="case-section-text"><p>${escapeHtml(text)}</p>${citationLink(selectedCase.source_url, 'Source link: original judgment on Indian Kanoon')}</div></details>`).join('')}</div><div class="research-actions"><button data-action="citation">Generate Citation</button><button data-action="export-summary">Export Summary</button><button data-action="compare">Compare with another case</button><button data-action="notes">Generate Research Notes</button><button data-action="download-pdf">Print / Save PDF</button></div><p class="source-attribution">Powered by Indian Kanoon</p>`;
}
