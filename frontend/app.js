...
function renderTreeBranches(nodes, parentSlug, depth = 1) {
  const children = nodes.filter(node => node.parent_slug === parentSlug);
  if (!children.length) return '';
  return `<div class="tree-children" style="--tree-depth:${depth}">${children.map(node => `<div class="tree-node ${node.is_current ? 'tree-current' : ''}"><span class="node-badge branch">${escapeHtml(node.relationship)}</span><h4>${escapeHtml(node.name)} (${escapeHtml(node.year)})</h4>${citationLink(node.source_url, 'Source link')}</div>${renderTreeBranches(nodes, node.slug, depth + 1)}`).join('')}</div>`;
}
...
function renderResearchPanel() {
  ...
  if (activeResearchTab === 'paragraphs') {
    root.innerHTML = `<div class="research-heading"><h3>Source Paragraphs</h3><p>Original paragraphs with concise source-grounded legal takeaways.</p></div>${paragraphs.length ? `<div class="paragraph-stream">${paragraphs.map((item, index) => `<article class="source-paragraph-card"><header><span class="para-label">Paragraph ${escapeHtml(item.paragraph_number)}</span><span class="para-classification-badge">${escapeHtml(item.classification)}</span></header><blockquote>${highlightLegalText(item.original_text, [...item.legal_terms, ...item.referenced_articles, ...item.referenced_acts, ...item.referenced_cases])}</blockquote><section class="inline-ai-insight"><h4>AI Analysis Summary</h4><ul>${paragraphInsightBullets(item).map(bullet => `<li>${escapeHtml(bullet)}</li>`).join('')}</ul></section><p class="granular-citation"><strong>Source reference:</strong> ${escapeHtml(item.citation_label)} · ${citationLink(item.source_url, 'Source link: original judgment on Indian Kanoon')}</p><button class="open-analysis-btn ${index === selectedParagraphIndex ? 'active' : ''}" data-paragraph="${index}">Open detailed analysis</button></article>`).join('')}</div>` : '<p class="empty-state">No source paragraphs were returned for this judgment.</p>'}`;
  } else if (activeResearchTab === 'analysis') {
    const paragraph = paragraphs[selectedParagraphIndex];
    root.innerHTML = paragraph ? `<div class="paragraph-analysis"><h3>Paragraph ${escapeHtml(paragraph.paragraph_number)}: ${escapeHtml(paragraph.classification)}</h3><section><h4>Original Paragraph</h4><blockquote>${highlightLegalText(paragraph.original_text, [...paragraph.legal_terms, ...paragraph.referenced_articles, ...paragraph.referenced_acts, ...paragraph.referenced_cases])}</blockquote><p class="granular-citation"><strong>Source reference:</strong> ${escapeHtml(paragraph.citation_label)} · ${citationLink(paragraph.source_url, 'Source link: original judgment on Indian Kanoon')}</p></section><section class="inline-ai-insight"><h4>AI Analysis Summary</h4><ul>${paragraphInsightBullets(paragraph).map(bullet => `<li>${escapeHtml(bullet)}</li>`).join('')}</ul><p class="granular-citation">${citationLink(paragraph.source_url, 'Source link: original judgment on Indian Kanoon')}</p></section><section><h4>Important Legal Terms</h4><div class="tag-row">${paragraph.legal_terms.map(value => `<span>${escapeHtml(value)}</span>`).join('') || '<span>No extracted terms</span>'}</div></section><section><h4>Referenced Authorities</h4><p><strong>Articles:</strong> ${escapeHtml(paragraph.referenced_articles.join(', ') || 'Not identified')}<br><strong>Acts:</strong> ${escapeHtml(paragraph.referenced_acts.join(', ') || 'Not identified')}<br><strong>Previous cases:</strong> ${escapeHtml(paragraph.referenced_cases.join(', ') || 'Not identified')}</p></section><section><h4>Why this paragraph is relevant</h4><p>${escapeHtml(paragraph.relevance)}</p><p class="granular-citation">${citationLink(paragraph.source_url, 'Source link: original judgment on Indian Kanoon')}</p></section></div>` : '<p class="empty-state">Select a source paragraph to view its analysis.</p>';
  } else if (activeResearchTab === 'tree') {
    const tree = selectedCase.heritage;
    const rootNode = tree?.nodes?.find(node => node.is_current);
    root.innerHTML = tree && rootNode ? `<div class="tree-wrapper"><div class="tree-node root-node"><span class="node-badge">Selected judgment</span><h4>${escapeHtml(tree.root.topics[0])}</h4><p>Up to ten live citation relationships across two precedent levels.</p>${citationLink(rootNode.source_url, 'Source link: original judgment on Indian Kanoon')}</div>${renderTreeBranches(tree.nodes, rootNode.slug)}</div>` : `<div class="empty-state">Citation lineage is unavailable. ${citationLink(selectedCase.source_url, 'Source link: original judgment on Indian Kanoon')}</div>`;
  } else if (activeResearchTab === 'similar') {
    const similar = selectedCase.similar || [];
    root.innerHTML = `<div class="research-heading"><h3>Similar Case Recommendation</h3><p>Direct citations supplemented by source-backed global legal search where needed.</p></div><div class="similar-list">${similar.length ? similar.map(item => `<div class="similar-card"><strong>${item.reason.startsWith('Related') ? 'Related' : 'Cited'}</strong><span>${escapeHtml(item.name)}<small>${escapeHtml(item.reason)}</small>${citationLink(item.source_url, 'Source link')}</span></div>`).join('') : `<div class="similar-card"><strong>Search</strong><span>Live related-case search did not return additional authorities.${citationLink(selectedCase.source_url, 'Source link: original judgment on Indian Kanoon')}</span></div>`}</div>`;
  } else {
    const graph = selectedCase.graph;
    root.innerHTML = graph ? `<div class="research-heading"><h3>Knowledge Graph</h3><p>${escapeHtml(graph.case)}</p></div><div class="knowledge-graph">${Object.entries(graph.nodes).map(([label, values]) => `<section><h4>${escapeHtml(label)}</h4>${values.length ? `<div class="tag-row">${values.map(value => `<span>${escapeHtml(value)}</span>`).join('')}</div>` : '<p>Not supplied by the live source.</p>'}${citationLink(selectedCase.source_url, 'Source link: original judgment on Indian Kanoon')}</section>`).join('')}</div>` : `<p class="empty-state">Knowledge graph data is unavailable. ${citationLink(selectedCase.source_url, 'Source link: original judgment on Indian Kanoon')}</p>`;
  }
}
